...
闭包存在有什么意义呢？为什么需要闭包？
```
我个人认为，闭包存在的意义就是它夹带了外部变量（私货），如果它不夹带私货，它和普通的函数就没有任何区别。同一个的函数夹带了不同的私货，就实现了不同的功能。其实你也可以这么理解，闭包和面向接口（接口定义了一套对方法签名的约束规则。）编程的概念很像，可以把闭包理解成轻量级的接口封装。
```
